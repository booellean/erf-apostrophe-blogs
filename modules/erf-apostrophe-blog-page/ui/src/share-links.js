export default () => {
    ...
    // document.querySelector('div#erf-test').innerHTML = ''+navigator.canShare()

    const tags = document.querySelector('#tags[data-tags]').dataset.tags.split(',')
    console.log(tags)
    ...
    `
    let shareDiv = window.shareDiv || null

    window.apos.util.widgetPlayers['erf-apostrophe-blog-page'] =
    {
        selector: '#'+shareDiv.id,
        player: function (el) {
          // ... use bigThing here
          console.log('playing...')
          if(isMobile){
            el.innerHTML = mobileIcons
          }else{
            el.innerHTML - articleSettings.icons
          }
          el.innerHTML = articleSettings.icons + mobileIcons
